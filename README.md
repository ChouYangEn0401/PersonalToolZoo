...
> **日誌診斷**：程式啟動時會在 console 列印每個語言檔的實際載入路徑（`[lang] Loaded ...`）。
> exe 執行時也會在 exe 旁生成 `githelper_language.log` 供診斷使用。
> 在程式中可透過 `from src.core.language_manager import lm; print(lm.last_loaded)` 查看最後載入的檔案。

#### `language_config.json`

儲存使用者選擇的語言代碼，位於 exe 旁（若目錄可寫）或使用者主目錄（`~/.githelper_config.json`）。
...
1. 複製 `language/_template.json` 為 `language/<語言代碼>.json`（例如 `jp.json`；檔名（不含 `.json`）即為語言代碼）
